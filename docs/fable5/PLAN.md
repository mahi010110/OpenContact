...
| V8 | Le Compagnon **réellement installable depuis l'app** : `release.yml` publie la GitHub Release (noms stables : deb + AppImage, NSIS, dmg universel ; chaque paquet FUMÉ — installé/lancé et canal sondé — avant publication) ; feuille « Ajouter le Compagnon » = télécharger pour SON système (liste réelle des assets, `engine/distribution.js`) → honnêteté non-signé → chercher → code → associer ; téléphone : chemin ordinateur + lien copiable + usage ensuite | Personne ne fouille dans GitHub Actions ; hors ligne/absent = repli honnête vers la page des releases | V7 | **terminée** | 88/88 unitaires (osFromUA, assetsForOS) ; E2E compagnon (installation guidée + repli + association) et audit UX (feuille téléphone) ; CSP corrigée (`api.github.com`, `openrouter.ai` — OpenRouter était mort en prod, E2E le verrouille) |
