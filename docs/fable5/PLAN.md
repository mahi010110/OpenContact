...
| P6-3 | Les trois familles D5 **complètes** : OpenRouter en clé navigateur (CORS confirmé chez le fournisseur) ; Ollama, OpenAI et l'abonnement ChatGPT « via ton ordinateur » — canal `ia-demarrer`/`ia-etat` du Compagnon (asynchrone, le canal reste vif), garde pure `oc_coeur::ia` (vocabulaire fermé, bornes, clé exigée), Codex officiel en mode non interactif documenté (bac à sable lecture seule, `--output-last-message`) ; plus aucune famille « pas encore disponible » | La clé OpenAI sert l'appel puis s'oublie — jamais écrite chez le Compagnon (ni disque ni journal) ; erreurs en codes courts traduits par la PWA ; texte toujours dans le champ éditable | P6-2, C2 | **terminée** | 4 tests Rust `oc_coeur::ia` + test moteur JS élargi + E2E `e2e-compagnon-ia.mjs` contre le vrai binaire (faux Ollama/OpenAI/Codex : Bearer vérifié, `grep` de la clé sur le disque du Compagnon = absente, mauvaise clé = refus court sans perte, Compagnon éteint = message honnête, 390×844 sombre + 1280×800 clair) |
